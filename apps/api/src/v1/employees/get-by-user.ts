...
import { EmployeesDetailsResponse } from "@hris-v2/contracts/employees"
...
const EmployeeByUserParams = z.object({
...
export function getEmployeeByUserIdRoute(
	_app: App,
	employeeRoute: OpenAPIHono,
) {
	employeeRoute.openapi(route, async (c) => {
		const { userId } = EmployeeByUserParams.parse(c.req.param())
