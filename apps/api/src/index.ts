...
import { ALLOWED_ORIGINS, API_PORT, BETTER_AUTH_URL } from "@/constants/env"
import { auth } from "@/lib/auth"
import { db, pg } from "@/lib/database"
import { getMailQueue } from "@/lib/email"
...
async function init() {
	...
	openAPIHono.openAPIRegistry.registerComponent(
		"securitySchemes",
		"CookieAuth",
		{
			type: "apiKey",
	...
	openAPIHono.doc("/openapi-doc", (_c) => {
		return {
			openapi: "3.0.0",
			info: {
				version: "1.0.0",
	...
	close(async ({ signal, err, manual }) => {
		...
		logger.info(`Server stopped with SIGNAL ${signal}. Manual: ${!!manual}`)
